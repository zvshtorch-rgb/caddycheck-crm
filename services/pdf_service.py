...
def _build_invoice_pdf_document(
    ...
    bold = _style("bold", fontSize=9, textColor=TEXT_DARK, leading=12, fontName="Helvetica-Bold")
    center_h1 = _style("ch1", fontSize=22, textColor=BLUE_DARK, leading=26, alignment=TA_CENTER)
    ...
    meta_data = [[
        ...
        "",
        Paragraph(f"Description: <b>{description}</b>", normal),
    ]]
    ...
    for row in line_items:
        ...
        subtotal += line_total
        units = row.get("num_cams")
        price = row.get("rate")
        tbl_data.append([
            row.get("project_name") or "",
            "" if units in (None, "") else str(int(units)),
            row.get("maintenance_year") or "",
            "" if price in (None, "") else f"€{float(price):,.0f}",
            f"€{line_total:,.0f}",
        ])
...
def generate_invoice_pdf(
    ...
    for row in preview:
        ...
        line_items.append({
            ...
            "maintenance_year": row["maintenance_year"],
            "rate": row["rate"],
            "line_total": row["line_total"],
        })

    return _build_invoice_pdf_document(
        line_items=line_items,
        invoice_number=invoice_number,
        description=f"Iretailcheck - Maintenance - {month_name[:3]} {year}",
    )


def generate_invoice_pdf_from_rows(
    invoice_rows: list,
    invoice_number: Optional[int] = None,
    ...
    line_items = []
    derived_years = []
    for row in sorted(invoice_rows, key=lambda item: str(getattr(item, "project_name", "")).lower()):
        project_name = str(getattr(row, "project_name", "") or "").strip()
        if not project_name:
            continue
        cameras_number = getattr(row, "cameras_number", None)
        try:
            cameras_number = int(cameras_number) if cameras_number not in (None, "") else None
        except Exception:
            cameras_number = None
        payment_amount = float(getattr(row, "payment_amount", 0) or 0.0)
        rate = None
        if cameras_number and cameras_number > 0:
            rate = payment_amount / cameras_number
        row_year = getattr(row, "year", None)
        if row_year is not None:
            derived_years.append(int(row_year))
        line_items.append({
            "project_name": project_name,
            "num_cams": cameras_number,
            "maintenance_year": str(getattr(row, "maintenance_year", "") or "").strip(),
            "rate": rate,
            "line_total": payment_amount,
        })

    if not line_items:
        raise ValueError("No invoice rows were available to build the PDF.")

    resolved_description = description or "Iretailcheck - Maintenance Adjustment"
    if derived_years and description is None:
        resolved_description = f"Iretailcheck - Maintenance Adjustment - {max(derived_years)}"
